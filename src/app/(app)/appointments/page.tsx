...
import { AppointmentCalendar } from "@/features/appointments/appointment-calendar";
...
export default async function AppointmentsPage({
  ...
  const [{ data: rows }, { data: customers }, { data: vehicles }, { data: services }, { data: technicians }] =
    await Promise.all([
      supabase
        .from("appointments")
        .select(
          "id,requested_service_summary,starts_at,ends_at,status,revision,customers(full_name),vehicles(plate_number)",
        )
  ...
  const events = (rows ?? []).map((row: any) => ({
    id: row.id,
    title: `${row.customers?.full_name ?? "Customer"} · ${row.vehicles?.plate_number ?? row.requested_service_summary}`,
    start: row.starts_at,
    end: row.ends_at,
    status: row.status,
  ...
  return (
    <section className="space-y-8">
      <PageHeader
        eyebrow="Scheduling"
        title="Appointments"
        description="Week calendar in Asia/Kabul time. Drag an appointment or open it to edit precise times."
        actions={
          <Link
            className="rounded-md bg-brand-primary px-4 py-2 text-sm font-medium text-brand-primary-foreground"
            href={`/appointments?date=${format(selected, "yyyy-MM-dd")}&create=1`}
          >
            New appointment
          </Link>
        }
      />
      <form className="flex items-center gap-2 rounded-lg border bg-card p-3">
        ...
        <button className="rounded-md border px-3 py-2 text-sm font-medium">Go</button>
        <Link className="text-sm font-medium underline" href="/appointments">
          Current day
        </Link>
      </form>
      {params.create === "1" ? (
      ...
      <AppointmentCalendar date={format(selected, "yyyy-MM-dd")} events={events} />
    </section>
  );
}
