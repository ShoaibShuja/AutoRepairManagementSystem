...
export default async function InventoryPage({
  ...
  let request = supabase
    .from("parts")
    .select("id,name,sku,category,unit,quantity_on_hand,reorder_threshold,sell_price_minor,archived_at", {
      count: "exact",
    })
    .order("name")
    .range((page - 1) * pageSize, page * pageSize - 1);
  if (filters.q) request = request.or(`name.ilike.%${filters.q}%,sku.ilike.%${filters.q}%`);
  ...
  if (filters.low === "true") request = request.filter("quantity_on_hand", "lte", "reorder_threshold");
  ...
  return (
    <section className="space-y-8">
      ...
      {filters.create === "1" ? (
        ...
        <a
          className="inline-flex rounded-md bg-brand-primary px-4 py-2 text-sm font-medium text-brand-primary-foreground"
          href="/inventory?create=1"
        >
          Add part
        </a>
      )}
      <form className="grid gap-2 rounded-lg border bg-card p-3 md:grid-cols-[1fr_auto_auto_auto_auto]">
        <input
        ...
        <label className="flex items-center gap-2 text-sm">
          <input defaultChecked={filters.low === "true"} name="low" type="checkbox" value="true" />
          Low stock
        </label>
        <button className="rounded-md border px-4 text-sm font-medium">Filter</button>
      </form>
      {error ? (
        <p className="text-destructive">Inventory could not be loaded.</p>
      ) : parts.length ? (
        <div className="overflow-x-auto rounded-lg border bg-card">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {parts.map((part) => (
                <tr className="border-b last:border-0" key={part.id as string}>
                  <td className="p-3">
                    <p className="font-medium">{part.name as string}</p>
                    <p className="text-xs text-muted-foreground">
                      {(part.sku as string) || "No SKU"}
                  ...
                  <td className="p-3">
                    ...
                    ) : Number(part.quantity_on_hand) <= Number(part.reorder_threshold) ? (
        ...
        <EmptyState title="No parts found" description="Add a part or adjust the inventory filters." />
      )}
