...
const pageSize = 20;
type WorkOrderListEntry = { id: string; public_number: number; status: string; created_at: string; assigned_technician_id: string | null; customers: { full_name: string; phone: string | null } | null; vehicles: { plate_number: string | null; make: string | null; model: string | null } | null; work_order_services: Array<{ unit_price_minor: number; quantity: number }> };
export default async function WorkOrdersPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; technician?: string; page?: string; create?: string }> }) {
  ...
  const [{ data: customers }, { data: vehicles }, { data: services }, { data: technicians }] = await Promise.all([
    ...
    supabase.from("vehicles").select("id, customer_id, plate_number, make, model").is("archived_at", null).order("created_at", { ascending: false }),
  ...
  return <section className="space-y-8"><PageHeader eyebrow="Operations" title="Work orders" description="Create, assign, and track workshop work. Service prices are captured when an order is created." actions={<Link className="rounded-md bg-brand-primary px-4 py-2 text-sm font-medium text-brand-primary-foreground" href="/work-orders?create=1">New work order</Link>} />
    ...
    {filters.create === "1" ? <WorkOrderForm customers={(customers ?? []).map((item) => ({ id:item.id, fullName:item.full_name, phone:item.phone }))} vehicles={(vehicles ?? []).map((item) => ({ id:item.id, customerId:item.customer_id, label:[item.make,item.model].filter(Boolean).join(" ") || "Vehicle", plate:item.plate_number }))} services={(services ?? []).map((item) => ({ id:item.id,name:item.name,description:item.description,price:item.standard_price_minor }))} technicians={(technicians ?? []).map((item) => ({ id:item.id,name:item.display_name }))} /> : null}
    {error ? <p className="text-destructive">Work orders could not be loaded.</p> : visibleOrders.length ? <div className="overflow-x-auto rounded-lg border bg-card"><table className="w-full text-left text-sm"><thead><tr className="border-b text-muted-foreground"><th className="p-3">Order</th><th className="p-3">Customer / vehicle</th><th className="p-3">Status</th><th className="p-3 text-right">Total</th></tr></thead><tbody>{visibleOrders.map((order) => <tr className="border-b last:border-0" key={order.id}><td className="p-3"><Link className="font-medium hover:underline" href={`/work-orders/${order.id}`}>WO-{String(order.public_number).padStart(6, "0")}</Link><p className="text-xs text-muted-foreground">{new Date(order.created_at).toLocaleDateString()}</p></td><td className="p-3">{order.customers?.full_name}<p className="text-xs text-muted-foreground">{order.vehicles?.plate_number || [order.vehicles?.make,order.vehicles?.model].filter(Boolean).join(" ")}</p></td><td className="p-3"><StatusBadge status={order.status === "in_progress" || order.status === "ready_for_review" || order.status === "assigned" || order.status === "draft" ? "pending" : order.status === "cancelled" ? "cancelled" : "completed"} /></td><td className="p-3 text-right"><CurrencyDisplay amount={order.work_order_services.reduce((sum, service) => sum + service.unit_price_minor * Number(service.quantity), 0)} /></td></tr>)}</tbody></table></div> : <EmptyState title="No work orders found" description="Create a work order or adjust the current filters." />}
    <p className="text-sm text-muted-foreground">Page {page} of {totalPages}</p>{page < totalPages ? <Link className="text-sm font-medium underline" href={`/work-orders?${new URLSearchParams({ ...filters, page: String(page + 1) })}`}>Next page</Link> : null}
  </section>;
}
