...
export default async function WorkOrderDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: availableParts }, { data: usedPartRows }, { data: attachmentRows }] = await Promise.all([
    supabase
      .from("parts")
      .select("id, name, sku, quantity_on_hand, unit")
      .is("archived_at", null)
      .order("name"),
    supabase
      .from("work_order_parts")
      .select("id, quantity, part_name_snapshot, unit_snapshot, reversed_at")
      .eq("work_order_id", id),
    supabase
      .from("attachments")
      .select("id, category, object_path, original_filename")
      .eq("work_order_id", id)
      .is("archived_at", null)
      .order("created_at", { ascending: false }),
  ]);
  ...
  const inputs = canEdit
    ...
    : null;
  const total = typed.work_order_services.reduce(
    (sum: number, service: any) => sum + service.unit_price_minor * Number(service.quantity),
    0,
  );
  const badge =
    typed.status === "cancelled"
      ? "cancelled"
      : ["completed", "invoiced"].includes(typed.status)
        ? "completed"
        : "pending";
  return (
    <section className="space-y-8">
      <PageHeader
        eyebrow="Work order"
        title={`WO-${String(typed.public_number).padStart(6, "0")}`}
        description={`${typed.customers?.full_name ?? "Customer"} · ${typed.vehicles?.plate_number ?? "Vehicle"}`}
        actions={<StatusBadge status={badge} />}
      />
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="space-y-6">
          <section className="rounded-lg border bg-card p-5">
            <h2 className="font-semibold">Customer concern</h2>
            <p className="mt-2 whitespace-pre-wrap text-sm text-muted-foreground">
              {typed.intake_notes || "No concern recorded."}
            </p>
            <h2 className="mt-5 font-semibold">Services</h2>
            <ul className="mt-2 space-y-2">
              {typed.work_order_services.map((service: any) => (
                <li className="rounded-md border p-3" key={service.id}>
                  <div className="flex justify-between gap-3">
                    <span className="font-medium">{service.service_name_snapshot}</span>
                    <CurrencyDisplay amount={service.unit_price_minor * Number(service.quantity)} />
                  </div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {service.service_description_snapshot || "No description"}
                  </p>
                </li>
              ))}
            </ul>
          ...
          <PartUsage
            workOrderId={typed.id}
            canUse={staff.role !== "technician" || typed.assigned_technician_id === staff.id}
            parts={(availableParts ?? []).map((part) => ({
              id: String(part.id),
              name: String(part.name),
              sku: part.sku == null ? null : String(part.sku),
              stock: Number(part.quantity_on_hand),
              unit: String(part.unit),
            }))}
            used={(usedPartRows ?? []).map((line) => ({
              id: String(line.id),
              name: String(line.part_name_snapshot),
              quantity: Number(line.quantity),
              unit: String(line.unit_snapshot),
              reversed: Boolean(line.reversed_at),
            }))}
          />
          <AttachmentPanel
            attachments={attachments}
            canManage={staff.role !== "technician" || typed.assigned_technician_id === staff.id}
            vehicleId={typed.vehicle_id}
            workOrderId={typed.id}
          />
          {staff.role !== "technician" && typed.status === "completed" ? (
            <form action={createInvoice}>
              <input name="workOrderId" type="hidden" value={typed.id} />
          ...
          {canEdit && inputs ? (
            <WorkOrderForm
              ...
              services={(inputs[2].data ?? []).map((item) => ({
                id: item.id,
                name: item.name,
              ...
              workOrder={{
                ...
                concern: typed.intake_notes,
                internalNotes: typed.technical_notes,
                estimatedCompletionAt: typed.estimated_completion_at,
                mileage: typed.reported_mileage,
                serviceIds: typed.work_order_services
                  .map((service: any) => service.service_catalog_id)
                  .filter(Boolean),
              }}
            />
          ) : null}
        ...
        <aside className="space-y-5">
          <section className="rounded-lg border bg-card p-5">
            ...
            <dl className="mt-3 space-y-2 text-sm">
              <div>
                ...
                <dd>
                  {[typed.vehicles?.model_year, typed.vehicles?.make, typed.vehicles?.model]
                    .filter(Boolean)
                    .join(" ") || typed.vehicles?.plate_number}
                </dd>
              </div>
              <div>
                <dt className="text-muted-foreground">Mileage</dt>
                <dd>
                  {typed.reported_mileage?.toLocaleString() ??
                    typed.vehicles?.mileage?.toLocaleString() ??
                    "Not recorded"}
                </dd>
              </div>
              <div>
                ...
                <dd>
                  {typed.estimated_completion_at
                    ? new Date(typed.estimated_completion_at).toLocaleString()
                    : "Not recorded"}
                </dd>
              </div>
            </dl>
          ...
          <section className="rounded-lg border bg-card p-5">
            ...
            {nextStatuses
              .filter((next) => canTransitionWorkOrder(staff.role, typed.status, next))
              .map((next) => (
                <form action={transitionWorkOrder} className="mt-3" key={next}>
                  <input name="workOrderId" type="hidden" value={typed.id} />
                  <input name="vehicleId" type="hidden" value={typed.vehicle_id} />
          ...
          <section>
            ...
            {activity?.length ? (
              ...
              <EmptyState
                ...
                description="Important work-order events will appear here."
              />
            )}
          </section>
          {staff.role !== "technician" ? (
            <Link className="text-sm font-medium underline" href={`/vehicles/${typed.vehicle_id}`}>
...
function TechnicianNoteForm({ workOrderId }: { workOrderId: string }) {
  return (
    <form action={addTechnicianNoteForm} className="rounded-lg border bg-card p-5">
      <input name="workOrderId" type="hidden" value={workOrderId} />
      <label className="text-sm font-medium">
        Technical note
        <textarea className="mt-2 min-h-24 w-full rounded-md border bg-background p-3" name="note" required />
      </label>
      <button className="mt-3 rounded-md bg-brand-primary px-4 py-2 text-sm font-medium text-brand-primary-foreground">
        Add note
      </button>
    </form>
  );
