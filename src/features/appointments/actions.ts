...
export type AppointmentActionState = { error?: string; message?: string };
...
function input(data: FormData) {
  return {
    ...
    serviceIds: data.getAll("serviceIds").map(String),
...
export async function transitionAppointment(formData: FormData) {
  ...
  await (
    await createClient()
  ).rpc("transition_appointment", {
    target_appointment_id: id,
    next_status: status(value(formData, "nextStatus")),
    expected_revision: Number(value(formData, "revision")),
  });
  revalidatePath("/appointments");
...
export async function convertAppointment(formData: FormData) {
  ...
  const { data, error } = await (
    await createClient()
  ).rpc("convert_appointment_to_work_order", {
    target_appointment_id: id,
    expected_revision: Number(value(formData, "revision")),
  });
  if (!error && data) redirect(`/work-orders/${data}`);
}
