...
export function WorkOrderForm({
  ...
  const [state, action, pending] = useActionState(
    workOrder ? updateWorkOrder : createWorkOrder,
    {} as WorkOrderActionState,
  );
  return (
    <form action={action} className="space-y-5">
      {workOrder ? <input name="workOrderId" type="hidden" value={workOrder.id} /> : null}
      <FormSection
        title={workOrder ? "Work-order details" : "New work order"}
        description="Catalog prices are copied into this order. Price adjustments are not available in this phase."
      >
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="text-sm font-medium">
            ...
            <input
              className="mt-1 h-10 w-full rounded-md border bg-background px-3"
              defaultValue={workOrder?.estimatedCompletionAt?.slice(0, 16) ?? ""}
          ...
          <label className="text-sm font-medium md:col-span-2">
            ...
            <textarea
              ...
              defaultValue={workOrder?.concern ?? ""}
              name="concern"
            />
          </label>
          <label className="text-sm font-medium md:col-span-2">
      ...
      <FormSection
        title="Services"
        description="Select one or more active catalog services. Saved name, description, and price stay attached to this work order."
      >
        <div className="grid gap-2 sm:grid-cols-2">
          {services.map((service) => (
            <label className="flex gap-3 rounded-md border p-3 text-sm" key={service.id}>
              <input
                defaultChecked={workOrder?.serviceIds.includes(service.id)}
                name="serviceIds"
                type="checkbox"
                value={service.id}
              />
