...
describe("authenticated UI foundation", () => {
  it("limits navigation visibility by staff role", () => {
    expect(getNavigationForRole("admin").map((item) => item.href)).toEqual(["/dashboard", "/customers", "/appointments", "/work-orders", "/services", "/staff"]);
    expect(getNavigationForRole("front_desk").map((item) => item.href)).toEqual(["/dashboard", "/customers", "/appointments", "/work-orders", "/services"]);
